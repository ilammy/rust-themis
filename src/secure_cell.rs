...
impl SecureCell {
    ...
    /// array, a `Vec<u8>`, or a `String`. Note that it is actually _a password_. It is not
    /// required to be an actual key generated by one of the [`keygen`] functions (though you
    /// definitely can use those keys as well):
    ///
    /// ```
    /// use themis::secure_cell::SecureCell;
    ///
...
/// authentication data and you absolutely need the output data to have same length as the
/// original input:
///
/// ```
/// # fn main() -> Result<(), themis::Error> {
...
/// some other means to validate the output.
